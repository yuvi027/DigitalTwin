...
sudo ovs-ofctl add-flow s1 ip,priority=1,nw_src=10.0.0.4,nw_dst=10.0.0.3,idle_timeout=0,actions=output:1
...
sudo ovs-ofctl add-flow s1 tcp,priority=10,tp_src=6000,idle_timeout=0,actions=output:2
...
# Traffic handling at s4
sudo ovs-ofctl add-flow s4 tcp,priority=10,tp_dst=6000,idle_timeout=0,actions=output:4
sudo ovs-ofctl add-flow s4 tcp,priority=10,tp_src=6000,idle_timeout=0,actions=output:4
sudo ovs-ofctl add-flow s4 ip,priority=1,nw_src=10.0.0.3,nw_dst=10.0.0.4,idle_timeout=0,actions=output:2
